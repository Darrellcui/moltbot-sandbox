...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  } else if (!env.ANTHROPIC_API_KEY && !env.QWEN_API_KEY) {
...
app.all('*', async (c) => {
  ...
  try {
    ...
    if (!c.env.ANTHROPIC_API_KEY && !c.env.QWEN_API_KEY) {
      hint = 'API Key is not set. Run: wrangler secret put ANTHROPIC_API_KEY or QWEN_API_KEY';
